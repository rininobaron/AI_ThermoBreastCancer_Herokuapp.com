...
def make_celery(flask_app):
    celery = Celery(flask_app.import_name, backend=flask_app.config['CELERY_RESULT_BACKEND'],
...
# CONECTANDO CON IBM WATSON CLOUD
...
def background(self, arreglo):
	...
	# Definimos la lista que se enviará para realizar la petición (Query) al modelo 1
	score_0 = PCA_transform(vect_dif).tolist()
	...
	try:
		...
		resultado_1 = np.around((get_score_model1(score_0))*100, 2)
		#resultado_1 = "IBM Watson Error ¡ESTAMOS TRABAJANDO EN ELLO!"
	...
	try:
		...
		# Redondeando a 2 decimales la probabilidad expresada en porcentaje
		resultado_2 = np.around((get_score_model2(score_1))*100, 2)
		#resultado_2 = "IBM Watson Error ¡ESTAMOS TRABAJANDO EN ELLO!"
